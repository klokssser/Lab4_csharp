Лабораторная работа №4: Коллекции и перегрузка операторов (Вариант 11)



Платформа: .NET 8.0 (Console Application)

Описание

Данный проект содержит решение задач лабораторной работы №4. Работа разделена на две логические части:

Коллекции (Задания 1-5): Работа со стандартными коллекциями .NET (List, LinkedList, HashSet, Dictionary) и файловый ввод/вывод.

Классы и операторы (Задания 6-7): Создание пользовательского типа данных LineSegment (Отрезок) и перегрузка операторов для него.

Часть 1: Коллекции (Класс CollectionTasks)

Этот статический класс содержит методы для решения задач 1–5.

1. Task1_ListIntersection (List)

Задача: Сформировать список, включив в него элементы, входящие одновременно в оба списка $L1$ и $L2$ (пересечение).

Метод: Используется LINQ метод Intersect.

Тест:
| Входные данные (L1) | Входные данные (L2) | Результат |
| :--- | :--- | :--- |
| [1, 2, 3, 4, 5] | [3, 4, 5, 6, 7] | [3, 4, 5] |

2. Task2_LinkedListPalindrome (LinkedList)

Задача: В конец списка добавить все его элементы в обратном порядке (создать палиндром).

Метод: Итерация исходного списка с конца и добавление элементов через AddLast.

Тест:
| Входные данные | Результат |
| :--- | :--- |
| [1, 2, 3] | [1, 2, 3, 3, 2, 1] |
| [10] | [10, 10] |

3. Task3_HashSetMalls (HashSet)

Задача: Анализ посещаемости ТРЦ студентами (множества).

Где были все.

Где был хотя бы один.

Где не был никто.

Метод: Используются операции над множествами: IntersectWith (пересечение), UnionWith (объединение), ExceptWith (разность).

4. Task4_FileOddWordsChars (HashSet + Файлы)

Задача: Вывести уникальные символы, встречающиеся в словах с нечётными порядковыми номерами.

Вход: Текстовый файл.

Логика: Текст разбивается на слова. Проверяется индекс слова (1, 3, 5...). Символы добавляются в HashSet<char> для уникальности и сортируются.

5. Task5_GasStations (Dictionary + Файлы)

Задача: Найти количество АЗС, продающих бензин (92, 95, 98) дешевле всего.

Структура данных: Dictionary<int, (int minPrice, int count)>.

Логика: При чтении файла обновляем минимальную цену или увеличиваем счетчик, если цена совпадает с текущим минимумом.

Часть 2: Класс LineSegment (Задания 6-7)

Пользовательский класс, описывающий отрезок на координатной прямой.
Поля: X, Y (координаты концов).
Особенность: При создании или изменении координат происходит автоматическая нормализация (гарантируется, что $X \le Y$).

Методы класса

IsIntersect(LineSegment other)

Проверяет пересечение двух отрезков.

Формула: max(x1, x2) <= min(y1, y2)

Тесты:
| Отрезок 1 | Отрезок 2 | Пересекаются? |
| :--- | :--- | :--- |
| [1; 5] | [2; 6] | true |
| [1; 2] | [3; 4] | false |
| [1; 2] | [2; 3] | true (точка касания) |

Перегрузка операторов (Вариант 11)

В соответствии с заданием реализованы следующие операторы:

Оператор

Тип

Описание

Пример кода

!

Унарный

Вычисляет длину отрезка ($

Y - X

++

Унарный

Расширяет отрезок на 1 в обе стороны ($X-1, Y+1$).

segment++;

implicit

Приведение

Неявное приведение к int. Возвращает целую часть $X$.

int x = segment;

explicit

Приведение

Явное приведение к double. Возвращает координату $Y$.

double y = (double)segment;

-

Бинарный

LineSegment - int. Уменьшает координату $X$.

var newSeg = segment - 5;

-

Бинарный

int - LineSegment. Уменьшает координату $Y$.

var newSeg = 10 - segment;

< / >

Бинарный

Проверяет пересечение отрезков (возвращает bool).

if (seg1 < seg2) ...

Примечание: Операторы сравнения < и > в C# должны быть перегружены парой. В данном контексте оба оператора проверяют факт пересечения.

Утилиты

Класс CheckInput

Статический класс для безопасного ввода данных с консоли без использования исключений (try-catch).

ReadDouble(message): Запрашивает ввод, пока не будет введено корректное число типа double. Поддерживает и точку, и запятую.

ReadInt(message): Запрашивает ввод, пока не будет введено целое число.

ReadString(message): Запрашивает непустую строку.

Запуск проекта

Откройте решение в Visual Studio или VS Code.

Убедитесь, что установлена версия .NET 8.0.

Запустите проект (F5 или dotnet run).

При первом запуске программа автоматически создаст тестовые файлы text_task4.txt и fuel_task5.txt в папке сборки.
